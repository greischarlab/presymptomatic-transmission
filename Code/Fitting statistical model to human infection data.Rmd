...
whichLower <- function(index, vec){
  ...
  # endPt is defined as the length of the full viral abundance time series
    ...
    # 2. any value prior to the endPt defined by (1) for which all subsequent values were greater
  ...
  if(any(minBeforeEnd) && any(remainingNonmissing[minBeforeEnd] > maxViralAbundance)){
    mins <- remainingNonmissing[minBeforeEnd]
    endPt <- min(mins[mins > maxViralAbundance])
  }
  ...
  if(endPt>=5 && endPt>maxViralAbundance+1){
    earlyLocalMinTest <- sapply(max(3, maxViralAbundance+1):(endPt-2), function(x) {
      left <- data$y_virus[max(c(1, x-2)):(x-1)]
  ...
  if(any(earlyLocalMinTest,na.rm=T)){
    endPt <- min(which(earlyLocalMinTest))+max(2, maxViralAbundance)
    }
  }
...
tiff('Norovirus trajectories (test).tiff', res=500, width=6.5, height=6.5, unit='in')
{
...
tiff('SARS-CoV-2 trajectories (test).tiff', res=500, width=6.5, height=6.5, unit='in')
...
{
  par(mfrow=c(2, 1), oma=c(0.1, 1.5, 0, 0), mai=c(1, 0.8, 0.3, 0.3))
  ...
  text(noro.example.first.date * 0.8, noro.example.params$avgSheddingBeforeSymptom * 0.85,
       labels="Avg shedding value\n before symptoms", cex=0.75,
       col=shading.col)
  text(2*0.95, 15*0.95, labels='A', font=2)
  ...
  text(sars.example.first.date * 3, sars.example.params$avgSheddingBeforeSymptom * 1.35,
       labels="Avg shedding value\n before symptoms", cex=0.75,
       col=shading.col)
  text(10*0.95, 15*0.95, labels='B', font=2)
...

{
  ...
  
  {
    ...
    mtext(text=expression(Duration~of~shedding~prior~to~symptoms~(log[10]~days)), side=1, line=1.5, cex=cex_labels)
