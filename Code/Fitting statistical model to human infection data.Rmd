...
noro.fits <- calc.measures(noro.fits, noroParticipants,
                                  noro.FShedData, noro.IncubData, numFits)

sars.fits <- calc.measures(sars.fits, sarsParticipants,
                                  sars.TShedData, sars.IncubData, numFits)
...
We then use bootstrapping via case resampling of the optimized 100 parameter fits 
per infection to create 95% confidence intervals (n = 1000) around the linear 
regressions we show in Figures 3 and S6 as well as the mean values per dose we 
show in Figures 3 and S4.
```{r} 
...
fitWeightings <- function(sseVector){
  weights <- 1 - sseVector/max(sseVector)
...
params.resampled <- function(fits) {
  ...
  indexValues <- 1:length(fits$sse)
  weightedFits <- fitWeightings(fits$sse)
  resampledIndices <- sample(indexValues, 1, 
...
We also calculate average shedding prior to symptoms and the duration of
...
Data on the volumes of each fecal sample are available by request from the 
authors; we do not provide them here.
```{r}
...

{
  ...
  for(i in 1: length(participantexamples)) {
    ...
    if (i == 1) {
      mtext("Peak after symptom onset", side=3, line=2, font=2)
    } else if (p==participantexamples[2]) {
      mtext("Peak before symptom onset\n(pre-symptomatic transmission)", side=3, 
            line=2, font=2)
    }
  }
