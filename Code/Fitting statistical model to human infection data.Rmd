...
# (noro.detection.threshold.2 copies /ml)
...
```{r}
shed.onset <- function(vldata, pars, t.range) {
  predValues <- sse.viralLoad(pars, vldata[1:t.range$endPt,], optimization = F)
    
  if("y_type" %in% colnames(vldata)) {
    threshold <- log10(noro.detection.threshold)
  } else {
    threshold <- log10(sars.detection.threshold)
  }

  return(min(predValues[which(predValues$predData >= threshold),]$t)/24)
}

total.shedding <- function(vldata) {
  
  if(length(vldata[grepl('~', vldata$y_type),]$y_virus) > 0) {
    vldata[grepl('~', vldata$y_type),]$y_virus <- mean(c(noro.detection.threshold, noro.detection.threshold.2))
  }
...
calc.measures <- function(params, endpoints, vldata, incubdata) {
  ...
  params$sheddingOnset <- sapply(params$ID, function(id) {
    shed.onset(vldata[vldata$ID == id, ],
               unlist(params[params$ID== id,][2:5]),
               endpoints[endpoints$ID == id, ])
  })
  ...
  params$totalConcentrationBeforeSymptoms <- sapply(params$ID, function(id) {
    total.shedding(vldata[(vldata$ID == id) & 
                            (vldata$days < params[params$ID==id,]$incubation), ])
  })
...
We remove measures depending on the fitted trajectories for participants with 
unclear peaks or not enough data points.
```{r}
...
onset is defined as when the viral load trajectory crosses the detection 
threshold for each pathogen.
```{r}
noro.avg.shed.onset <- mean(noro.params$sheddingOnset, na.rm=T)
noro.avg.shed.onset

noro.avg.symp.onset <- mean(noro.IncubData$incubation, na.rm=T)
noro.avg.symp.onset

sars.avg.shed.onset <- mean(sars.params$sheddingOnset, na.rm=T)
sars.avg.shed.onset
...
presymp.transmission.potential <- function(vldata, sympdata, measure='concentration') {
  ...
  if(length(vl.presymp[grepl('~', vl.presymp$y_type),]$y_virus) > 0) {
    vl.presymp[grepl('~', vl.presymp$y_type),]$y_virus <- mean(c(noro.detection.threshold, noro.detection.threshold.2))
  }
  
  if(measure == 'total shedding') {
    avg.presymp.shedding <- mean(log10(vl.presymp$total_shedding), na.rm=T)
  } else {
    avg.presymp.shedding <- mean(log10(vl.presymp$y_virus), na.rm=T)
  }
...
add.presymp.transmission.potential <- function(params, vldata, incubdata) {
  
  presymptrans.avg <- apply(params, 1, function(x) 
    presymp.transmission.potential(vldata[vldata$ID == x[1],],
                                   incubdata[incubdata$ID == x[1],]))
...
noro.totalshedding$total_shedding <- ifelse(grepl('<', noro.totalshedding$quantity),
                                                  mean(c(noro.detection.threshold,
                                                         noro.detection.threshold.2)) * noro.totalshedding$weight,
                                                  as.numeric(noro.totalshedding$quantity) * noro.totalshedding$weight)

# Average total shedding values at the same time point. For those rows,
# total_shedding will no longer simply equal quantity * weight, because we have 
# gotten rid of duplicate days and replaced individual total_shedding values with 
# an aggregate.
...
lm.fit.p4.p2.conservative.noro <- 
  lm(p4 ~ p2, data=noro.params[!noro.params$ID %in% noro.excluded])
...
lm.fit.p4.p2.conservative.sars <- 
  lm(p4 ~ p2, data=sars.params[!sars.params$ID %in% sars.excluded])
...
lm.fit.p2.symponset.noro <- lm(incubation ~ p2, data=noro.params[!noro.params$ID %in% noro.excluded])
...
lm.fit.p2.symponset.sars <- lm(incubation ~ p2, data=sars.params[!sars.params$ID %in% sars.excluded])
...
plot.viralload <- function(vldata, pars, sympdata, t.range, cex=1,
  ...
  if(metric=='concentration') {yvals <- vldata$y_virus} 
  else {yvals <- vldata$total_shedding}
  
  points(vldata$days, log10(yvals), col=col, cex=cex, pch=pch)
...
tiff('Norovirus trajectories.tiff', res=500, width=6.5, height=6.5, unit='in')
{
...
measure.names <- c("ID", "dose", "incubation", "sheddingOnset", "repRate",
                   ...
                   "totalConcentration", "totalConcentrationBeforeSymptoms")
     ...
     width=3.5, unit='in')
{
  ...
  
  {
    ...
    legend(-0.85, 
       ...
       pch=c(19, 1), col=c("black", "black", "black"), cex=0.9, bty="n")
  }
}
dev.off()
```
...

{
  ...
  
  {
    ...
    legend(-0.875, 
       ...
       pch=c(19, 1, 9), col=c("black", "black", "black"), cex=0.85, bty="n")
  }
}
...
Now, we plot cumulative log10(viral concentration) as well as cumulative 
log10(viral concentration) prior to symptoms against viral replication rate.
```{r}
totshed_model <- lm(totalConcentration ~ repRate,
                    data=measures.combined)

totshedpresymp_model <- lm(totalConcentrationBeforeSymptoms ~ repRate,
                       data=measures.combined)

yrange <- c(min(measures.combined$totalConcentrationBeforeSymptoms),
...
{
  par(mfrow=c(3, 1), mai=c(0.4, 0.8, 0.4, 0.2))
  ...
  segments(noro.example.vldata$days,
           log10(1e-16),
           noro.example.vldata$days,
           ifelse(grepl('~', noro.example.vldata$y_type),
                  log10(mean(c(noro.detection.threshold, 
                               noro.detection.threshold.2))),
                  log10(noro.example.vldata$y_virus)),
           col=noro.color)
  
  usr <- par("usr")
  
  par(xpd=NA)
  bracket_y <- usr[4] + 0.05 * (usr[4]-usr[3])
  bracket_tick_length <- 0.02 * (usr[4] - usr[3])
  
  # Brackets before symptoms
  segments(0, bracket_y, noro.example.params$incubation - 0.25, bracket_y)
  
  segments(0, bracket_y, 0, bracket_y - bracket_tick_length)
  
  segments(noro.example.params$incubation - 0.25, bracket_y,
           noro.example.params$incubation - 0.25,
          bracket_y - bracket_tick_length)
  
  points((noro.example.params$incubation - 0.25)/2, 
         bracket_y + bracket_tick_length*2,
         pch=0)
  
  # Brackets overall
  segments(0, 
           bracket_y + bracket_tick_length * 4, 
           20, 
           bracket_y + bracket_tick_length * 4)
  ...
  segments(0, 
           bracket_y + bracket_tick_length * 4, 
           0, 
           bracket_y + bracket_tick_length * 3)
  
  segments(20, bracket_y + bracket_tick_length * 4,
           20,
          bracket_y + bracket_tick_length * 3)
  
  points(20/2, bracket_y + bracket_tick_length * 6,
         pch=1)
  ...
  plot.viralload(vldata=sars.example.vldata, 
  ...
  segments(sars.example.vldata$days,
           log10(1e-16),
           sars.example.vldata$days,
           log10(sars.example.vldata$y_virus),
           col=sars.color)
  ...
  # Brackets before symptoms
  segments(0, bracket_y, sars.example.params$incubation - 0.25, bracket_y)
  
  segments(0, bracket_y, 0, bracket_y - bracket_tick_length)
  
  segments(sars.example.params$incubation - 0.25, bracket_y,
           sars.example.params$incubation - 0.25,
          bracket_y - bracket_tick_length)
  
  points((sars.example.params$incubation - 0.25)/2, 
         bracket_y + bracket_tick_length*2,
         pch=0)
  ...
  points(15/2, bracket_y + bracket_tick_length * 6,
         pch=1)
  
  par(xpd=FALSE)
  
  # Cumulative concentration and pre-symptomatic cumulative concentration
  # vs. viral replication rate
  ...
  axis(1, at=seq(-1.25, 0, by=0.25),
       ...
       mgp=c(0, 0.5, 0), tck=-0.04, cex.axis=1.1)
  
  mtext(text=expression(Viral~replication~rate~(hour^-1)),
            side=1, line=2.25, cex=0.9)
  ...
  axis(2, at=seq(0, 250, by=50),
       ...
       mgp=c(0, 0.5, 0), tck=-0.04, las=2, cex.axis=1.1)
  
  mtext(text='Cumulative viral shedding', side=2, line=3, cex=0.9)
  
  points(totalConcentrationBeforeSymptoms ~ repRate, data=measures.combined,
        col=ifelse(pathogen == 'Norovirus', noro.color, sars.color),
        pch=ifelse(symptomsToPeakDay < 0, 15, 0), cex=1.1)
  
  segments(x0=noro.params$repRate, 
           y0=noro.params$totalConcentrationBeforeSymptoms,
           y1=noro.params$totalConcentration,
           col=noro.color)
  
  segments(x0=sars.params$repRate, 
           y0=sars.params$totalConcentrationBeforeSymptoms,
...
We plot average shedding prior to symptoms without factoring in sample weights vs. 
the duration of shedding prior to symptoms for participants from both the norovirus
and SARS-CoV-2 study. Panel A and B show examples of this calculation.
```{r}
...
noro.example.first.date <- min(noro.example.vldata[noro.example.vldata$days!=0, ]$days)
...
sars.example.first.date <- min(sars.example.vldata[sars.example.vldata$days!=0, ]$days)

shading.col <- '#A37A74'
panellabelratio <- 0.95

tiff("Average shedding prior to symptoms.tiff", res=500, height=6.5,
     width=3.5, unit="in")

{
  ...
  text(noro.example.params$incubation*.7, 15*.9, labels='Symptom onset',
       col=noro.color)
  ...
  text(sars.example.params$incubation*.77, 15*.9, labels='Symptom onset',
       col=sars.color)
  ...
  segments(x0 = sars.example.first.date,
           ...
           col=shading.col, lty=3, lwd=1.5)
  text(sars.example.first.date * 2.8, sars.example.params$avgSheddingBeforeSymptom * 1.35,
       labels="Average shedding\nbefore symptoms", 
       col=shading.col)

  # Pre-symptomatic shedding vs. duration of pre-symptomatic shedding
  
    xrange <- c(floor(min(measures.combined$sheddingTimeBeforeSymptoms,
                    na.rm=T)) - 1,
                ceiling(max(measures.combined$sheddingTimeBeforeSymptoms,
                    na.rm=T)) + 1)
    ...
    plot(avgSheddingBeforeSymptoms ~ sheddingTimeBeforeSymptoms, 
         data=measures.combined,
         col=ifelse(pathogen == 'Norovirus', noro.color, sars.color), 
         pch=ifelse(symptomsToPeakDay < 0 & pathogen == 'Norovirus', 17, 
                ifelse(symptomsToPeakDay > 0 & pathogen == 'Norovirus', 2,
    ...
    mtext(text="Duration of shedding before symptoms (days)", side=1, line=1.75, cex=0.8)
    
    # Lines of best fit
    noro.presymp.dur.range <- seq(min(noro.params$sheddingTimeBeforeSymptoms, na.rm=T),
                            max(noro.params$sheddingTimeBeforeSymptoms, na.rm=T),
                            length=10)
    ...
    sars.presymp.dur.range <- seq(min(sars.params$sheddingTimeBeforeSymptoms, na.rm=T),
                            max(sars.params$sheddingTimeBeforeSymptoms, na.rm=T),
                            length=10)
...
{
  {
    par(mfrow=c(2, 1), oma=c(0.1, 2.5, 0, 0), mai=c(1, 0.8, 0.1, 0.1))
    ...
    segments(x0 = example.first.date,
             ...
             col=shading.col, lty=3, lwd=1.5)
    text(example.first.date, noro.example.params$avgTotalSheddingBeforeSymptom * 0.9,
  ...
  
  {
    ...
    noro.presymp.dur.range <- seq(min(noro.params$sheddingTimeBeforeSymptoms,
                                      na.rm=T),
                            max(noro.params$sheddingTimeBeforeSymptoms,
                                na.rm=T),
                            length=10)
    # Lines of best fit
