...
# not exact values
noro.detection.threshold <- noro.detection.threshold
noro.detection.threshold.2 <- noro.detection.threshold.2
...
To determine endpoints for model fitting, we take the first local minimum 
in the data. If such a value does not exist, we take the first value lesser than 
...
transmission. The delay is in days. We also calculate average viral concentration 
prior to symptoms and the duration of shedding prior to symptoms for each person,
as well as average viral concentration over the course of infection and the
duration of shedding overall.
...
shed.duration <- function(vldata) {
  if("y_type" %in% colnames(vldata)) {
    return(max(vldata[grepl('POS', vldata$y_type),]$days) - shed.onset(vldata))
  } else {
    return(max(vldata[!is.na(vldata$y_virus),]$days) - shed.onset(vldata))
  }
}
...
total.shedding <- function(vldata) {
  ...
  if('y_type' %in% colnames(vldata)) {
    ...
    if(length(vldata[grepl('~', vldata$y_type),]$y_virus) > 0) {
      vldata[grepl('~', vldata$y_type),]$y_virus <- 10^mean(log10(c(noro.detection.threshold,
                                                                      noro.detection.threshold.2)))
    }

  } 

  return(sum(vldata$y_virus, na.rm=T))
...
calc.measures <- function(params, endpoints, vldata, incubdata) {
  ...
  params$totalSheddingBeforeSymptoms <- apply(params, 1, function(p) {
    total.shedding(vldata[vldata$ID == p["ID"] & vldata$days < as.numeric(p["incubation"]),])})
 
  params$sheddingTimeOverall <- sapply(params$ID, function(id) {
    shed.duration(vldata[vldata$ID == id, ])})
  
  params$avgSheddingOverall <- apply(params, 1, function(p) {
    avg.shedding(vldata[vldata$ID == p["ID"] & vldata$days <= 
                          as.numeric(p["sheddingOnset"]) + as.numeric(p["sheddingTimeOverall"])
                        & vldata$days >= as.numeric(p["sheddingOnset"]),])})
  
  params$transmissionOverall <- log10(10^params$avgSheddingOverall * 
                                                 params$sheddingTimeOverall)
  
  params$totalSheddingOverall <- apply(params, 1, function(p) {
    total.shedding(vldata[vldata$ID == p["ID"],])})
...
We remove measures for participants with unclear peaks in their trajectories or 
...
rep.summary.table <- function(params, iters, n_points) {
  ...
  r.range <- seq(min(params$repRate, na.rm=T),
                    max(params$repRate, na.rm=T),
                    length=n_points)

  summary <- data.frame(repRate = r.range,
...
noroSummary.rep <- rep.summary.table(noro.params,
                                 1000, 10)

sarsSummary.rep <- rep.summary.table(sars.params,
                                 1000, 10)
...
We do the same for SARS-CoV-2, barring delay vs. dose.
```{r}
...
There is also no correlation between average shedding overall and the
duration of shedding overall.
```{r}
...
transmission.avg.fit.noro <- lm(avgSheddingOverall ~ sheddingTimeOverall, 
                              data = noro.params)
...
transmission.avg.fit.sars <- lm(avgSheddingOverall ~ sheddingTimeOverall, 
                              data = sars.params)
...
There is no correlation between transmission potential prior to symptoms (
average shedding prior to symptoms times incubation period) and
viral replication rate for either norovirus or SARS-CoV-2. There is, however,
a correlation between overall transmission potential (average shedding over 
the duration of shedding times duration of shedding) and replication rate for
norovirus. This pattern is not exhibited in the SARS-CoV-2 data.
```{r}
lm.fit.p2.presymp.transmission.noro <- lm(transmissionBeforeSymptoms ~ repRate, 
                              data = noro.params)
...
lm.fit.p2.presymp.transmission.sars <- lm(transmissionBeforeSymptoms ~ repRate, 
                              data = sars.params)
...
lm.fit.p2.overall.transmission.noro <- lm(transmissionOverall ~ repRate, 
                              data = noro.params)
...
lm.fit.p2.overall.transmission.sars <- lm(transmissionOverall ~ repRate, 
                              data = sars.params)
...
plot.viralload <- function(vldata, pars, sympdata, t.range, cex=1,
                           xaxisLabs = T, yaxisLabs = T, col, pch, xmax=25, ylim.lower=3,
...
tiff('Norovirus trajectories.tiff', res=500, width=6.5, height=7, unit='in')
{
...
tiff('SARS-CoV-2 trajectories.tiff', res=500, 
...
delay vs. viral replication rate, day of peak shedding vs. viral replication rate, and
day of symptom onset vs. viral replication rate in a 4-panel plot.
```{r, fig.height = 7.5, fig.width = 3.5}
measure.names <- c("ID", "dose", "incubation", "sheddingOnset", "repRate",
                   ...
                   "avgSheddingOverall", "sheddingTimeOverall",
                   "transmissionBeforeSymptoms", "transmissionOverall",
                   "totalSheddingBeforeSymptoms", "totalSheddingOverall")
...
noro.r.range <- seq(min(noro.params$repRate, na.rm=T),
                    max(noro.params$repRate, na.rm=T),
                    length=10)

sars.r.range <- seq(min(sars.params$repRate, na.rm=T),
                    max(sars.params$repRate, na.rm=T),
                    length=10)
     ...
     width=3.5, unit='in')
{
  ...
  
  {
    ...
    plot(symptomsToPeakDay ~ repRate, data=measures.combined, 
     col=ifelse(pathogen == 'Norovirus', noro.color, sars.color),
     pch=ifelse(symptomsToPeakDay < 0 & pathogen == 'Norovirus', 17, 
                  ifelse(symptomsToPeakDay > 0 & pathogen == 'Norovirus', 2,
                         ifelse(symptomsToPeakDay < 0 & pathogen == 'SARS-CoV-2', 23, 
    ...
    lines(sympToPeakDay ~ repRate, data=noroSummary.rep, col=noro.color)
    lines(sympToPeakDay ~ repRate, data=sarsSummary.rep, col=sars.color)
  ...
  
  {
    ...
    plot(peakDay ~ repRate, data=measures.combined,
       col=ifelse(pathogen == 'Norovirus', noro.color, sars.color),
       xlim=xlim.bcd,
    ...
    # Lines of best fit
    lines(peakDay ~ repRate, data=noroSummary.rep, col=noro.color)
  
    lines(peakDay ~ repRate, data=sarsSummary.rep, col=sars.color)
  ...
  
  {
    ...
    plot(incubation ~ repRate, data=measures.combined,
         col=ifelse(pathogen == 'Norovirus', noro.color, sars.color), 
    ...
    # Lines of best fit
    lines(symp ~ repRate, data=noroSummary.rep, col=noro.color)
  
    lines(symp ~ repRate, data=sarsSummary.rep, col=sars.color)
...
tiff('Non-conservative pre-symptomatic transmission analysis.tiff', res=500, 
...

{
  ...
  {  
    ...
    legend(0.35, 
           ...
           legend=c("Shedding onset before symptoms", 
                    "Shedding onset after symptoms", 
                    "Shedding onset = symptom onset"), 
    ...
    
  {
    ...
    plot(symptomsToStartShedding ~ repRate, data=measures.combined, 
       col=ifelse(pathogen == 'Norovirus', noro.color, sars.color),
       pch=ifelse(symptomsToStartShedding < 0 & pathogen == 'Norovirus', 17, 
                    ifelse(symptomsToStartShedding > 0 & pathogen == 'Norovirus', 2,
                           ifelse(symptomsToStartShedding < 0 & pathogen == 'SARS-CoV-2', 23, 
    ...
    # Lines of best fit
    lines(sympToShed ~ repRate, data=noroSummary.rep, col=noro.color)
  
    lines(sympToShed ~ repRate, data=sarsSummary.rep, col=sars.color)
  ...
  
  {
    ...
    plot(sheddingOnset ~ repRate, data=measures.combined,
       col=ifelse(pathogen == 'Norovirus', noro.color, sars.color),
       xlim=xlim.bcd,
       ylim=ylim.c, cex.axis=cex, cex=cex.points,
       pch=ifelse(symptomsToStartShedding < 0 & pathogen == 'Norovirus', 17, 
    ...
    # Lines of best fit
    lines(shedding ~ repRate, data=noroSummary.rep, col=noro.color)
  
    lines(shedding ~ repRate, data=sarsSummary.rep, col=sars.color)
...
- average shedding throughout infection vs. duration of shedding
- cumulative log10(viral concentration) vs. viral replication rate
- cumulative log10(viral concentration) prior to symptoms vs. viral replication
rate
...
noshedding.col <- "#525252"

tiff('Average shedding and transmission potential.tiff', res=500,
...

{
  ...
  segments(x0 = noro.example.params$sheddingOnset,
           y0 = noro.example.params$avgSheddingOverall, 
           x1 = noro.example.params$sheddingOnset + noro.example.params$sheddingTimeOverall,
  ...
  segments(x0 = sars.example.params$sheddingOnset,
           y0 = sars.example.params$avgSheddingOverall, 
           x1 = sars.example.params$sheddingOnset + 
             sars.example.params$sheddingTimeOverall,
  ...
  plot(avgSheddingBeforeSymptoms ~ incubation, 
       data=measures.combined,
       col=ifelse(pathogen == 'Norovirus', noro.color, sars.color), 
       bg=ifelse(pathogen == 'Norovirus', noro.color, sars.color),
       pch=ifelse(!is.na(symptomsToPeakDay) & symptomsToPeakDay < 0 & 
                    pathogen == 'Norovirus', 17, 
                ifelse(!is.na(symptomsToPeakDay) & symptomsToPeakDay > 0 & 
                         pathogen == 'Norovirus', 2,
                       ifelse(!is.na(symptomsToPeakDay) & symptomsToPeakDay < 0 & 
                                pathogen == 'SARS-CoV-2', 23, 
                              ifelse(!is.na(symptomsToPeakDay) & symptomsToPeakDay > 0 & 
                                       pathogen == 'SARS-CoV-2', 5,
                                     9)))),
       ylim=yrange,
       xlim=xrange,
       cex=1,
  ...
  axis(2, cex.axis=1.2, mgp=c(0, 0.7, 0), tck=-0.04, at=seq(yrange[1], yrange[2], by=2.5),
       labels=do.call(expression, ylabels),
       las=2)
  
  abline(h=0, lty=3, col=noshedding.col)
  
  text(6.5, -0.7, 'No shedding before symptoms', col=noshedding.col)
  ...
  arrows(x0 = noro.example.params$incubation,
         y0 = noro.example.params$avgSheddingBeforeSymptoms - 2,
         x1 = noro.example.params$incubation,
         y1 = noro.example.params$avgSheddingBeforeSymptoms - 0.5,
         length=0.1,
         col="red")
  
  arrows(x0 = sars.example.params$incubation,
         y0 = sars.example.params$avgSheddingBeforeSymptoms - 2,
         x1 = sars.example.params$incubation,
         y1 = sars.example.params$avgSheddingBeforeSymptoms - 0.5,
         length=0.1,
         col="red")
  ...
  xrange <- c(floor(min(measures.combined$sheddingTimeOverall)) - 1,
              ceiling(max(measures.combined$sheddingTimeOverall)) + 5)
  yrange <- c(floor(min(measures.combined$avgSheddingOverall)) - 1,
              ceiling(max(measures.combined$avgSheddingOverall)) + 1)

  plot(avgSheddingOverall ~ sheddingTimeOverall, 
       data=measures.combined,
       col=ifelse(pathogen == 'Norovirus', noro.color, sars.color), 
       pch=ifelse(!is.na(symptomsToPeakDay) & symptomsToPeakDay < 0 & 
                    pathogen == 'Norovirus', 17, 
  ...
  mtext(text="Duration of shedding (days)", side=1, line=2,
        cex=0.9)
  
  # Lines of best fit
  noro.shed.dur.range <- seq(min(noro.params$sheddingTimeOverall, na.rm=T),
                          max(noro.params$sheddingTimeOverall, na.rm=T),
                          length=10)
  
  noro.dpred <- data.frame(sheddingTimeOverall= noro.shed.dur.range)
  noro.dpred <- transform(noro.dpred, y_predicted = predict(transmission.avg.fit.noro, 
                                                          newdata=noro.dpred))
  lines(y_predicted ~ sheddingTimeOverall, data=noro.dpred, col=noro.color)
  
  sars.shed.dur.range <- seq(min(sars.params$sheddingTimeOverall, na.rm=T),
                          max(sars.params$sheddingTimeOverall, na.rm=T),
                          length=10)

  sars.dpred <- data.frame(sheddingTimeOverall = sars.shed.dur.range)
  sars.dpred <- transform(sars.dpred, y_predicted = predict(transmission.avg.fit.sars, 
                                                           newdata=sars.dpred))
  lines(y_predicted ~ sheddingTimeOverall, data=sars.dpred, col=sars.color)
  
  arrows(x0 = noro.example.params$sheddingTimeOverall,
         y0 = noro.example.params$avgSheddingOverall - 2,
         x1 = noro.example.params$sheddingTimeOverall,
         y1 = noro.example.params$avgSheddingOverall - 0.5,
         length=0.1,
         col="red")
  
  arrows(x0 = sars.example.params$sheddingTimeOverall,
         y0 = sars.example.params$avgSheddingOverall - 2,
         x1 = sars.example.params$sheddingTimeOverall,
         y1 = sars.example.params$avgSheddingOverall - 0.5,
         length=0.1,
         col="red")
  ...
  legend(15, 6, 
         legend=c("Peak shedding before symptoms", "Peak shedding after symptoms",
                  "Unclear peak or no symptoms"), 
     pch=c(19, 1, 9), col=c("black", "black", "black"), bty="n")
  
  # Panel E: Pre-symptomatic transmission potential vs. viral replication rate
  
  yrange=c(floor(min(measures.combined$transmissionBeforeSymptoms, na.rm=T)) - 1,
         ceiling(max(measures.combined$transmissionBeforeSymptoms, na.rm=T)) + 1)
  
  plot(transmissionBeforeSymptoms ~ repRate, data=measures.combined,
    col=ifelse(pathogen == 'Norovirus', noro.color, sars.color),
    bg=ifelse(pathogen == 'Norovirus', noro.color, sars.color),
    pch=ifelse(symptomsToPeakDay < 0 & pathogen == 'Norovirus', 17, 
                ifelse(symptomsToPeakDay > 0 & pathogen == 'Norovirus', 2,
  ...
  text(-0.8, -0.75, 'No shedding before symptoms', col=noshedding.col)
  
  mtext(text='Transmission potential\nprior to symptoms', side=2, line=2.5,
        cex=0.9)
  
  arrows(x0 = noro.example.params$repRate,
         y0 = noro.example.params$transmissionBeforeSymptoms - 2,
         x1 = noro.example.params$repRate,
         y1 = noro.example.params$transmissionBeforeSymptoms - 0.5,
         length=0.1,
         col="red")
  
  arrows(x0 = sars.example.params$repRate,
         y0 = sars.example.params$transmissionBeforeSymptoms - 2,
         x1 = sars.example.params$repRate,
         y1 = sars.example.params$transmissionBeforeSymptoms - 0.5,
         length=0.1,
         col="red")
  ...
  # Panel F: Overall transmission potential vs. viral replication rate
  
  yrange=c(floor(min(measures.combined$transmissionOverall)) - 1,
         ceiling(max(measures.combined$transmissionOverall)) + 1)
  
  plot(transmissionOverall ~ repRate, data=measures.combined,
    col=ifelse(pathogen == 'Norovirus', noro.color, sars.color),
    bg=ifelse(pathogen == 'Norovirus', noro.color, sars.color),
    pch=ifelse(!is.na(symptomsToPeakDay) & symptomsToPeakDay < 0 & 
                    pathogen == 'Norovirus', 17, 
  ...
  ylabels <- c(bquote(10^5),
               bquote(10^7),
               bquote(10^9),
               bquote(10^11))
  
  axis(2, at=seq(5, 12, by=2),
       labels=do.call(expression, ylabels),
       mgp=c(0, 0.7, 0), tck=-0.04, las=2, cex.axis=1.2)
  ...
  arrows(x0 = noro.example.params$repRate,
         y0 = noro.example.params$transmissionOverall - 2,
         x1 = noro.example.params$repRate,
         y1 = noro.example.params$transmissionOverall - 0.5,
         length=0.1,
         col="red")
  
  arrows(x0 = sars.example.params$repRate,
         y0 = sars.example.params$transmissionOverall - 2,
         x1 = sars.example.params$repRate,
         y1 = sars.example.params$transmissionOverall - 0.5,
         length=0.1,
         col="red")
...
the duration of shedding prior to symptoms for participants from Ge *et al.*, 
2023. Panel A shows an example of this calculation.
```{r}
example.first.date <- min(noro.FShedData[noro.FShedData$ID==exampleIDs[1] &
                                           noro.FShedData$days>0, ]$days)

tiff("Average total shedding prior to symptoms.tiff", res=500, height=6.5,
     width=3.5, unit="in")

{
  ...
  
  {
    ...
    plot(avgTotalSheddingBeforeSymptoms ~ incubation,
         data=noro.params,
         col=noro.color,
    ...
    axis(2, mgp=c(0, 0.5, 0), tck=-0.04, at=seq(yrange[1], yrange[2], by=2.5),
         labels=do.call(expression, ylabels),
         las=2)
    
    abline(h=0, lty=3, col=noshedding.col)
  
    text(1.25, -0.7, 'No shedding before symptoms', col=noshedding.col, cex=0.8)
...
We also plot percentage of observed shedding values before symptoms against
viral replication rate.
```{r}
par(mar=c(5.1, 5.1, 4.1, 2.1))
plot(totalSheddingBeforeSymptoms / totalSheddingOverall ~ repRate, data=measures.combined,
      col=ifelse(pathogen == 'Norovirus', noro.color, sars.color), 
       pch=ifelse(symptomsToPeakDay < 0 & pathogen == 'Norovirus', 17, 
                  ifelse(symptomsToPeakDay > 0 & pathogen == 'Norovirus', 2,
                         ifelse(symptomsToPeakDay < 0 & pathogen == 'SARS-CoV-2', 23, 
                                ifelse(symptomsToPeakDay > 0 & pathogen == 'SARS-CoV-2', 5,
                                       9)))),
       bg=ifelse(pathogen == 'Norovirus', noro.color, sars.color),
     bty='n', xlab=expression(Viral~replication~rate~(hour^-1)), 
     ylab="Proportion of total shedding\nbefore symptoms")
...

{
  ...
  # Viral replication rate vs. dose
  {
    ...
    ylim <- c(floor(min(noro.params$repRate)) - 1,
                ceiling(max(noro.params$repRate)) + 1)
  ...
  
  {
    ...
    ylim <- c(floor(min(noro.params$peakDay)) - 1,
                ceiling(max(noro.params$peakDay)) + 1)
...
combinedNoroData <- combinedNoroData[!is.na(combinedNoroData$peakDay),]
...
# Ordering by geID
combinedNoroData <- combinedNoroData[order(as.numeric(substring(combinedNoroData$geID, 3, 4))),]

atmarColor <- "#f1a340"
geColor <- "#871a6e"

tiff('Symptom onset discrepancy.tiff', res=500, width=6.5, height=3.5, unit='in')
{
