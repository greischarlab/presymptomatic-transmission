...
```{r}
calc.measures <- function(params, participants, shedding.data,
                           incub.data) {
  
  n_participants <- length(participants)
  
  params <- merge(params, incub.data, by="ID")
  ...
  params$sheddingOnset <- sapply(params$ID, function(id) {
    shed.onset(shedding.data[shedding.data$ID == id, ])
  })
...

{
  ...
  
  {
    ...
    mtext(text=expression(Duration~of~shedding~prior~to~symptom~onset~(log[10]~days)), side=1, line=1.5, cex=cex_labels)
