...
whichLower <- function(index, vec){
  ...
  endPt <- length(vldata$y_virus)
  
  localMaxTest <- sapply(3:(endPt-2), function(x) {
      left <- vldata$y_virus[(x-2):(x-1)]
      right <- vldata$y_virus[(x+1):(x+2)]
      
      all(left < vldata$y_virus[x]) && all(right < vldata$y_virus[x])
  ...
  if(any(localMaxTest,na.rm=T)){
    ...
    minIndex <- which.max(vldata$y_virus) + 1
  }
  ...
  runs <- rle(is.na(vldata$y_virus))
  endIndices <- cumsum(runs$lengths)
  startIndices <- endIndices - runs$lengths + 1
  
  missingRuns <- which(runs$values)
  
  nextIndex <- pmin(endIndices[missingRuns] + 1, length(vldata$days))
  gaps <- vldata$days[nextIndex] - 
    vldata$days[startIndices[missingRuns]]
  ...
  remainingNonmissing <- which(is.finite(vldata$y_virus[minIndex:endPt])) + minIndex - 1
  minBeforeEnd <- sapply(seq_len(max(length(remainingNonmissing)-1, 0)), 
                         whichLower,
                         vec = vldata$y_virus[remainingNonmissing])
  ...
  if(endPt>3 && endPt>minIndex){
    ...
    earlyLocalMinTest <- sapply(max(3, minIndex):
                                  min(length(vldata$y_virus)-2, (endPt-1)), 
                                function(x) {
      left <- vldata$y_virus[(x-2):(x-1)]
      right <- vldata$y_virus[(x+1):(x+2)]
...
sse.viralLoad <- function(pars, vldata, detect.threshold, 
                          detect.threshold.2 = NA, optimization=T) {
  
  vl <- log10(vldata$y_virus)
  t <- vldata$days * 24
  
  finite.range <- 24*range(vldata$days[is.finite(vldata$y_virus)],na.rm=T)
  
  p1 <- exp(pars[["p1"]])
  p2 <- exp(pars[["p2"]])
  p3 <- exp(-exp(pars[["p3"]]))*diff(finite.range) + finite.range[1]
  p4 <- exp(pars[["p4"]])
  
  if(is.na(optimization) | optimization==F){
    # return hi resolution predicted time series
      hiResT <- seq(min(vldata$days[vldata$days > 0]) * 24, 
                   max(vldata$days) * 24, by=1)
  ...
  if(!is.na(optimization) & optimization == T){
    ...
    # qualitative results
    vl[which(grepl('~', vldata$y_type) & estimates < detect.threshold)] <- detect.threshold 
    vl[which(grepl('~', vldata$y_type) & estimates > detect.threshold.2)] <- detect.threshold.2
    
    # Keep NA if estimates lie between two thresholds
    vl[which(grepl('~', vldata$y_type) & estimates < detect.threshold.2 & estimates > detect.threshold)] <- NA
...
optim.params <- function(parm.guess, vldata, detect.threshold,
                                     detect.threshold.2 = NA) {
  
  fit <- optim(parm.guess, sse.viralLoad, vldata=vldata, detect.threshold=detect.threshold,
                 detect.threshold.2=detect.threshold.2)
  
  transformed.params <- sse.viralLoad(fit$par, vldata = vldata, optimization = NA)
  param.guess <- data.frame(ID=unique(vldata$ID), 
                         p1=fit$par[1],
                         p2=fit$par[2],
                         p3=fit$par[3],
                         p4=fit$par[4],
...
eval.sse <- function(vldata, startguesses, detect.threshold, detect.threshold.2 = NA) {

  evalAtStartParms <- apply(startguesses, 1, sse.viralLoad, vldata = vldata, 
                            detect.threshold, detect.threshold.2)
  
  res <- as.data.frame(cbind(rep(vldata$ID[1],length(startguesses[,1])), startguesses, evalAtStartParms))
...
minsse <- function(fits) {
  ...
  minsse <- min(fits$sse)
  pars <- (fits[fits$sse == minsse, ])[1,]
  return(pars)
  
}

all.evals <- function(vldata, endpoints, startguess, detect.threshold, detect.threshold.2 = NA) {
  
  participants <- unique(vldata$ID)

  participantfits <- do.call(rbind,
                             lapply(participants,
                                    function(x) 
                                      eval.sse(vldata[vldata$ID==x,]
...
all.fits <- function(vldata, endpoints, startguess, numFits,
  ...
  for (p in participants) {
      ...
      for (i in 1:nrow(pguess)) {
        ...
        fit <- optim.params(pg[parmColumns], 
                            vldata[vldata$ID == p,][1:(endpoints[endpoints$ID==p,]$endPt),],
                            detect.threshold, 
                            detect.threshold.2)
...
delay between shedding onset and symptom onset, where shedding onset is defined
as the first time that each person's viral load trajectory reaches the 
lowest detection threshold. A negative delay indicates 
pre-symptomatic transmission, while a positive one indicates post-symptomatic
transmission. The delay is in days.

We also calculate AUC using the integrate() function to approximate transmission 
potential before symptoms and transmission potential until the end of the first 
viral load 'hump,' defined as the endpoint of the fitted trajectory, for each 
person.
...
auc.shedding <- function(pars, vldata, detect.threshold, endpoint=NA, presymp = F) {
  
  if (presymp) {
    if(is.na(endpoint)) {return(NA)} 
  }
  ...
  fit.range <- seq(0, max(vldata$days) * 24, by=1)
  vl.trajectory <- data.frame(t=fit.range, predData = generate.estimates(fit.range, p1, p2, p3, p4))
  positive.vals <- vl.trajectory[vl.trajectory$predData >= log10(detect.threshold),]

  to.integrate <- approxfun(x = positive.vals$t, y = positive.vals$predData)
  
  if (presymp) {
    if (endpoint * 24 < positive.vals$t[1]) {return(0)}
    
    res <- integrate(to.integrate,
                   positive.vals$t[1],
                   endpoint*24)$value
  } else {
    res <- integrate(to.integrate,
                   positive.vals$t[1],
                   positive.vals$t[nrow(positive.vals)])$value
  }
  
  # Convert to log10(viralload) * days
  return(res/24)
  
}

calc.measures <- function(params, endpoints, vldata, incubdata, detect.threshold) {
  ...
  params$avgSheddingBeforeSymptoms <- purrr::pmap_dbl(params, 
                                                  function(...) {
                                                    row <- list(...)
                                                    avg.shedding(vldata[vldata$ID == row$ID & 
                                                                          vldata$days < row$incubation & 
                                                                          vldata$days > 0,])})

  params$transmissionBeforeSymptoms <- purrr::pmap_dbl(params, 
                                                      function(...) {
                                                        row <- list(...)
                                                        auc.shedding(c(p1=row$p1, p2=row$p2, p3=row$p3, p4=row$p4),
                                                                     vldata[vldata$ID == row$ID,][
                                                                       1:endpoints[endpoints$ID == row$ID,]$endPt,],
                                                                     detect.threshold,
                                                                     row$incubation,
                                                                     presymp=T
                                                                     )})
                                             
  params$endFirstWave <- sapply(params$ID, function(id) {
    vldata[vldata$ID == id,][endpoints[endpoints$ID==id,]$endPt,]$days
  })
  
  params$transmissionFirstWave <- purrr::pmap_dbl(params, 
                                             function(...) {
                                               row <- list(...)
                                               auc.shedding(c(p1=row$p1, p2=row$p2, p3=row$p3, p4=row$p4),
                                                                     vldata[vldata$ID == row$ID,][
...
noro.params <- calc.measures(noro.best.fit, noro.endpoints, noro.FShedData, 
                             noro.IncubData, noro.detection.threshold)

noro.params$sheddingEnd <- sapply(noro.params$ID, function(id) {
      max(noro.FShedData[noro.FShedData$ID == id & grepl('POS', noro.FShedData$y_type),]$days)})
    
noro.params$avgSheddingOverall <- purrr::pmap_dbl(noro.params, function(...) {
  row <- list(...) 
  avg.shedding(noro.FShedData[noro.FShedData$ID == row$ID & 
                                noro.FShedData$days > 0 & 
                                noro.FShedData$days <= row$sheddingEnd,])})

sars.params <- calc.measures(sars.best.fit, sars.endpoints, sars.TShedData, 
                             sars.IncubData, sars.detection.threshold)
```
...
```{r}
positivePts <- function(vldata){
  quantifiedAbundance <- is.finite(vldata$y_virus)
  positiveDetection <- quantifiedAbundance
  if("y_type" %in% colnames(vldata)){positiveDetection <- grepl("POS", vldata$y_type)}
...
noro.params[which(noro.params$ID%in%noro.unclearpeaks),][, c('transp1',
                                                              ...
                                                              'peakDay',
                                                              'symptomsToPeakDay',
                                                             'transmissionBeforeSymptoms',
                                                             'endFirstWave',
                                                             'transmissionFirstWave')] <- NA
...
```{r} 
lmMetrics <- function(dataSet, predictorVar, predictorRange, responseVar){
  lmFit <- lm(formula = reformulate(predictorVar, response = responseVar), data = dataSet)

  newdata <- setNames(data.frame(predictorRange), predictorVar)
  
  res <- predict(lmFit, newdata = newdata)
  return(res)
} 

ci.band.linreg <- function(params, predictorVar, responseVar, iters, n_points) {
  
  params <- params[!is.na(params[responseVar]) & !is.na(params[predictorVar]),]
  
  predictorRange <- seq(min(params[predictorVar]),
                    max(params[predictorVar]),
                    length=n_points)
  
  summary <- setNames(data.frame(predictorRange), predictorVar)
  summary[responseVar] <- lmMetrics(params, predictorVar, predictorRange, responseVar)
  
  resamples <- matrix(NA, nrow = iters, ncol = n_points)
  ...
  for(i in 1:iters){
    ...
    resamples[i, ] <- lmMetrics(params.resampled, predictorVar, predictorRange, responseVar)
      
  }
  
  summary[paste(responseVar, 'LCI', sep='')] <- apply(resamples, MARGIN = 2, FUN = function(c) quantile(c, 0.025))
  summary[paste(responseVar, 'UCI', sep='')] <- apply(resamples, MARGIN = 2, FUN = function(c) quantile(c, 0.975))
  
  return(summary)
}

ci.band.dose <- function(params, responseVar, iters) {
  
  doses <- unique(params$dose)
  
  params <- params[!is.na(params[responseVar]),]
  
  summary <- NULL
    
  for (d in doses) {
    
    resamples.bydose <- matrix(NA, nrow = iters, ncol = 1)
    params.bydose <- params[params$dose == d,]
    
    for (i in 1:iters) {
      params.resampled <- params.bydose[
      sample(1:nrow(params.bydose), 
             size=nrow(params.bydose),
             replace=T),]
      
      resamples.bydose[i, ] <- mean(params.resampled[[responseVar]])
      
    }
    
    lci <- quantile(resamples.bydose, 0.025)
    uci <- quantile(resamples.bydose, 0.975)

    summary.bydose <- setNames(data.frame(d, lci, uci), c('dose', paste(responseVar, 'LCI', sep=''),
                                                          paste(responseVar, 'UCI', sep='')))
    summary <- rbind(summary, summary.bydose)
  }
  
  summary <- merge(summary, as.data.frame(
    params %>%
      group_by(dose) %>%
      summarise("{responseVar}" := mean(.data[[responseVar]]))
  ), by='dose')
    
  return(summary)
}

# Linear regression against viral population growth rate confidence bands

growthRate.responseVars <- c('peakDay', 'incubation', 'symptomsToPeakDay',
                             'sheddingOnset', 'symptomsToStartShedding',
                             'transmissionBeforeSymptoms', 'transmissionFirstWave')

noro.ci.growth <- lapply(growthRate.responseVars, function(x) {ci.band.linreg(noro.params, 
                                                                                  'growthRate',
                                                                                  x, 1000, 10)})

noro.ci.growth <- reduce(noro.ci.growth, merge, by='growthRate')

sars.ci.growth <- lapply(growthRate.responseVars, function(x) {ci.band.linreg(sars.params, 
                                                                                  'growthRate',
                                                                                  x, 1000, 10)})

sars.ci.growth <- reduce(sars.ci.growth, merge, by='growthRate')

# Viral load AUC versus symptom onset and viral load AUC versus end of first infection wave 
# confidence bands

noro.ci.auc.incub <- ci.band.linreg(noro.params, 'incubation', 
                                    'transmissionBeforeSymptoms',
                                    1000, 10)

noro.ci.auc.firstWave <- ci.band.linreg(noro.params, 'endFirstWave',
                                        'transmissionFirstWave',
                                        1000, 10)

sars.ci.auc.incub <- ci.band.linreg(sars.params, 'incubation', 
                                    'transmissionBeforeSymptoms',
                                    1000, 10)

sars.ci.auc.firstWave <- ci.band.linreg(sars.params, 'endFirstWave',
                                        'transmissionFirstWave',
                                        1000, 10)

# Average values per inoculum dose confidence bands


dose.responseVars.noro <- c('symptomsToPeakDay', 'symptomsToStartShedding',
                            'incubation', 'peakDay', 'growthRate')

noro.ci.dose <- lapply(dose.responseVars.noro, function(x) {ci.band.dose(noro.params, 
                                                                             x, 
                                                                             1000)})

noro.ci.dose <- reduce(noro.ci.dose, merge, by='dose')

dose.responseVars.sars <- c('symptomsToPeakDay', 'symptomsToStartShedding')

sars.ci.dose <- lapply(dose.responseVars.sars, function(x) {ci.band.dose(sars.params, 
                                                                             x, 
                                                                             1000)})

sars.ci.dose <- reduce(sars.ci.dose, merge, by='dose')
...
noro.params$avgTotalSheddingOverall <- purrr::pmap_dbl(noro.params, function(...) {
  row <- list(...)
  avg.shedding(noro.FShedData[noro.FShedData$ID == row$ID &
                                noro.FShedData$days > 0 &
                                noro.FShedData$days <= row$sheddingEnd, ],
...
# Transmission prior to symptoms (AUC) vs. incubation period
lm.fit.incub.presymp.auc.noro <- lm(transmissionBeforeSymptoms ~ incubation, 
                              data = noro.params)
incub.presymp.auc.pval.noro <- 
  summary(lm.fit.incub.presymp.auc.noro)$coefficients[2,4]
incub.presymp.auc.pval.noro

incub.presymp.auc.rsquared.noro <-
  summary(lm.fit.incub.presymp.auc.noro)$r.squared
incub.presymp.auc.rsquared.noro

# Transmission during first wave of infection (AUC) vs. duration of first wave
lm.fit.dur.firstwave.auc.noro <- lm(transmissionFirstWave ~ endFirstWave, 
                              data = noro.params)
dur.firstwave.auc.pval.noro <- 
  summary(lm.fit.dur.firstwave.auc.noro)$coefficients[2,4]
dur.firstwave.auc.pval.noro

# Transmission prior to symptoms (AUC) vs. viral population growth rate
lm.fit.p2.presymp.auc.noro <- lm(transmissionBeforeSymptoms ~ growthRate, 
                              data = noro.params)
p2.presymp.auc.pval.noro <- 
  summary(lm.fit.p2.presymp.auc.noro)$coefficients[2,4]
p2.presymp.auc.pval.noro

# Transmission during first wave of infection (AUC) vs. viral population growth rate
lm.fit.p2.firstwave.auc.noro <- lm(transmissionFirstWave ~ growthRate, 
                              data = noro.params)
p2.firstwave.auc.pval.noro <- 
  summary(lm.fit.p2.firstwave.auc.noro)$coefficients[2,4]
p2.firstwave.auc.pval.noro
...
# Transmission prior to symptoms (AUC) vs. incubation period
lm.fit.incub.presymp.auc.sars <- lm(transmissionBeforeSymptoms ~ incubation, 
                              data = sars.params)
incub.presymp.auc.pval.sars <- 
  summary(lm.fit.incub.presymp.auc.sars)$coefficients[2,4]
incub.presymp.auc.pval.sars

# Transmission during first wave of infection (AUC) vs. duration of first wave
lm.fit.dur.firstwave.auc.sars <- lm(transmissionFirstWave ~ endFirstWave, 
                              data = sars.params)
dur.firstwave.auc.pval.sars <- 
  summary(lm.fit.dur.firstwave.auc.sars)$coefficients[2,4]
dur.firstwave.auc.pval.sars

# Transmission prior to symptoms (AUC) vs. viral population growth rate
lm.fit.p2.presymp.auc.sars <- lm(transmissionBeforeSymptoms ~ growthRate, 
                              data = sars.params)
p2.presymp.auc.pval.sars <- 
  summary(lm.fit.p2.presymp.auc.sars)$coefficients[2,4]
p2.presymp.auc.pval.sars

# Transmission during first wave of infection (AUC) vs. viral population growth rate
lm.fit.p2.firstwave.auc.sars <- lm(transmissionFirstWave ~ growthRate, 
                              data = sars.params)
p2.firstwave.auc.pval.sars <- 
  summary(lm.fit.p2.firstwave.auc.sars)$coefficients[2,4]
p2.firstwave.auc.pval.sars
...
plot.viralload <- function(vldata, pars, sympdata, t.range, cex=1,
                           ...
                           example=FALSE, metric='concentration', detect.threshold=NA,
                           auc.endpoint=NA) {
  
  # Load up trajectories
  if(metric=='concentration') {
    if(example) {
      predValues <- sse.viralLoad(pars, vldata[0:t.range$endPt,], optimization = F)
    } else {
  ...
  if(example) {
    # Plot AUC
    positive.vals <- predValues[predValues$predData >= log10(detect.threshold),]
    
    if(!is.na(auc.endpoint)) {
      auc.linet <- positive.vals[positive.vals$t <= auc.endpoint * 24,]$t
      auc.predictedvl <- positive.vals[positive.vals$t <= auc.endpoint * 24,]$predData
    } else {
      auc.linet <- positive.vals$t
      auc.predictedvl <- positive.vals$predData
    }
    polygon(x=c(auc.linet / 24, rev(auc.linet / 24)),
            y=c(rep(0, length(auc.linet)), rev(auc.predictedvl)),
            col="#e69f00",
            border=NA)
  }
  ...
  # Symptom onset and end
  symp.onset <- sympdata$incubation
  
  if ('y_type' %in% colnames(vldata) || 
      ('symptomEnd' %in% colnames(sympdata) && any(is.na(sympdata$symptomEnd)))) {
    symp.end <- xmax
  } else {
    symp.end <- sympdata$symptomEnd
  }
  
  rect(symp.onset, ylim.lower, symp.end, 15,
...
tiff('Figure 2.tiff', res=500, width=6.5, height=7, unit='in')
{
  ...
  for(i in 1:length(noroParticipants)) {
    ...
    params <- noro.params[noro.params$ID==p,]
...
```{r, fig.height = 7.5, fig.width = 3.5}
params.combined <- rbind(noro.params[!colnames(noro.params) %in% c('p1', 'p2', 
                                                                     'p3', 'p4', 
                                                                     'sse', 
                                                                     'transp1', 
                                                                     'transp2', 
                                                                     ...
                                                                     'sheddingEnd',
                                                                   'avgSheddingOverall',
                                                                    'avgTotalSheddingOverall')],
                           sars.params[!colnames(sars.params) %in% c('p1', 'p2', 
                                                                     'p3', 'p4', 
                                                                     'sse', 
                                                                     'transp1', 
                                                                     'transp2', 
                                                                     ...
                                                                     'transp4',
                                                                     'symptomEnd')])
params.combined$pathogen <- ifelse(grepl('ID', params.combined$ID), 
     ...
     width=3.5, unit='in')
{
  ...
  {
    ylim.ab <- c(floor(min(params.combined$symptomsToPeakDay, na.rm=T))-1, 
               ceiling(max(params.combined$symptomsToPeakDay, na.rm=T))+1)
    ...
    segments(0.63, 
             noro.ci.dose[noro.ci.dose$dose == 4.8,]$symptomsToPeakDay, 
             0.83, 
             noro.ci.dose[noro.ci.dose$dose == 4.8,]$symptomsToPeakDay, 
             col=noro.color,
             lwd=cex)
    ...
    segments(0.967, 
             noro.ci.dose[noro.ci.dose$dose == 48,]$symptomsToPeakDay, 
             1.167,
             noro.ci.dose[noro.ci.dose$dose == 48,]$symptomsToPeakDay, 
             col=noro.color,
             lwd=cex)
    ...
    segments(1.3, 
             noro.ci.dose[noro.ci.dose$dose == 4800,]$symptomsToPeakDay, 
             1.5,
             noro.ci.dose[noro.ci.dose$dose == 4800,]$symptomsToPeakDay, 
             col=noro.color,
             lwd=cex)
    ...
    segments(1.9, 
             sars.ci.dose$symptomsToPeakDay, 
             2.1,
             sars.ci.dose$symptomsToPeakDay, col=sars.color,
             lwd=cex)
    
    # Confidence bounds
     polygon(c(0.63, 0.83, 0.83, 0.63), c(noro.ci.dose[noro.ci.dose$dose == 4.8,]
                                         $symptomsToPeakDayLCI,
                                         noro.ci.dose[noro.ci.dose$dose == 4.8,]
                                         $symptomsToPeakDayLCI,
                                         noro.ci.dose[noro.ci.dose$dose == 4.8,]
                                         $symptomsToPeakDayUCI,
                                         noro.ci.dose[noro.ci.dose$dose == 4.8,]
                                         $symptomsToPeakDayUCI),
            col=rgb(135, 26, 110, alpha=100,
                  maxColorValue=255), border = NA)
    
    polygon(c(0.967, 1.167, 1.167, 0.967), c(noro.ci.dose[noro.ci.dose$dose == 48,]
                                             $symptomsToPeakDayLCI,
                                             noro.ci.dose[noro.ci.dose$dose == 48,]
                                             $symptomsToPeakDayLCI,
                                             noro.ci.dose[noro.ci.dose$dose == 48,]
                                             $symptomsToPeakDayUCI,
                                             noro.ci.dose[noro.ci.dose$dose == 48,]
    ...
    polygon(c(1.3, 1.5, 1.5, 1.3), c(noro.ci.dose[noro.ci.dose$dose == 4800,]
                                         $symptomsToPeakDayLCI,
                                         noro.ci.dose[noro.ci.dose$dose == 4800,]
                                         $symptomsToPeakDayLCI,
                                         noro.ci.dose[noro.ci.dose$dose == 4800,]
                                         $symptomsToPeakDayUCI,
                                         noro.ci.dose[noro.ci.dose$dose == 4800,]
    ...
    polygon(c(1.9, 2.1, 2.1, 1.9), c(sars.ci.dose$symptomsToPeakDayLCI,
                                         sars.ci.dose$symptomsToPeakDayLCI,
                                         sars.ci.dose$symptomsToPeakDayUCI,
                                         sars.ci.dose$symptomsToPeakDayUCI),
  ...
  
  {
    ...
    plot(symptomsToPeakDay ~ growthRate, data=params.combined, 
     col=ifelse(pathogen == 'Norovirus', noro.color, sars.color),
     pch=ifelse(symptomsToPeakDay < 0 & pathogen == 'Norovirus', 17, 
                  ifelse(symptomsToPeakDay > 0 & pathogen == 'Norovirus', 2,
                         ifelse(symptomsToPeakDay < 0 & pathogen == 'SARS-CoV-2', 23, 
    ...
    lines(symptomsToPeakDay ~ growthRate, data=noro.ci.growth, col=noro.color)
    lines(symptomsToPeakDay ~ growthRate, data=sars.ci.growth, col=sars.color)
    
    # Confidence bounds 
    polygon(c(noro.ci.growth$growthRate, rev(noro.ci.growth$growthRate)), 
            c(noro.ci.growth$symptomsToPeakDayUCI, rev(noro.ci.growth$symptomsToPeakDayLCI)), 
          col = rgb(135, 26, 110, alpha=100,
                  maxColorValue=255), border = NA)
    
    polygon(c(sars.ci.growth$growthRate, rev(sars.ci.growth$growthRate)), 
            c(sars.ci.growth$symptomsToPeakDayUCI, rev(sars.ci.growth$symptomsToPeakDayLCI)), 
          col=rgb(86, 179, 233, alpha=100, maxColorValue=255), border = NA)
  ...
  {
    ylim.c <- c(floor(min(params.combined$peakDay, na.rm=T)) - 1, 
                ceiling(max(params.combined$peakDay, na.rm=T)) + 1)
    
    plot(peakDay ~ growthRate, data=params.combined,
       col=ifelse(pathogen == 'Norovirus', noro.color, sars.color),
       xlim=xlim.bcd,
    ...
    # Lines of best fit
    lines(peakDay ~ growthRate, data=noro.ci.growth, col=noro.color)
    lines(peakDay ~ growthRate, data=sars.ci.growth, col=sars.color)
    
    # Confidence bounds 
    polygon(c(noro.ci.growth$growthRate, rev(noro.ci.growth$growthRate)), 
            c(noro.ci.growth$peakDayUCI, rev(noro.ci.growth$peakDayLCI)), 
          col=rgb(135, 26, 110, alpha=100,
                  maxColorValue=255), border = NA)
    
    polygon(c(sars.ci.growth$growthRate, rev(sars.ci.growth$growthRate)), 
            c(sars.ci.growth$peakDayUCI, rev(sars.ci.growth$peakDayLCI)), 
            col=rgb(86, 179, 233, alpha=100,
                  maxColorValue=255), border = NA)
  ...
  {
    ylim.d <- c(floor(min(params.combined[!params.combined$ID %in% 
                                                c(noro.unclearpeaks, sars.unclearpeaks),]
                          $incubation, na.rm=T)) - 1, 
              ceiling(max(params.combined[!params.combined$ID %in% 
                                                c(noro.unclearpeaks, sars.unclearpeaks),]
                          $incubation, na.rm=T)) + 1)
  
    plot(incubation ~ growthRate, data=params.combined,
         col=ifelse(pathogen == 'Norovirus', noro.color, sars.color), 
    ...
    # Lines of best fit
    lines(incubation ~ growthRate, data=noro.ci.growth, col=noro.color)
    lines(incubation ~ growthRate, data=sars.ci.growth, col=sars.color)
    
    # Confidence bounds 
    polygon(c(noro.ci.growth$growthRate, rev(noro.ci.growth$growthRate)), 
            c(noro.ci.growth$incubationUCI, rev(noro.ci.growth$incubationLCI)), 
            col = rgb(135, 26, 110, alpha=100,
                    maxColorValue=255), border = NA)
    
    polygon(c(sars.ci.growth$growthRate, rev(sars.ci.growth$growthRate)), 
            c(sars.ci.growth$incubationUCI, rev(sars.ci.growth$incubationLCI)), 
            col=rgb(86, 179, 233, alpha=100,
                    maxColorValue=255), border = NA)
...

{
  ...
  {  
    ylim.ab <- c(floor(min(params.combined$symptomsToStartShedding, na.rm=T))-1, 
                 ceiling(max(params.combined$symptomsToStartShedding, na.rm=T))+1)
    ...
    #Average delays per pathogen
    segments(0.63, 
             noro.ci.dose[noro.ci.dose$dose == 4.8,]$symptomsToStartShedding, 
             0.83,
             noro.ci.dose[noro.ci.dose$dose == 4.8,]$symptomsToStartShedding, 
             col=noro.color,
             lwd=cex)
    ...
    segments(0.967, 
             noro.ci.dose[noro.ci.dose$dose == 48,]$symptomsToStartShedding, 
             1.167,
             noro.ci.dose[noro.ci.dose$dose == 48,]$symptomsToStartShedding, 
             col=noro.color,
             lwd=cex)
    ...
    segments(1.3, 
             noro.ci.dose[noro.ci.dose$dose == 4800,]$symptomsToStartShedding, 
             1.5,
             noro.ci.dose[noro.ci.dose$dose == 4800,]$symptomsToStartShedding, 
             col=noro.color,
             lwd=cex)
    
    segments(1.9, sars.ci.dose$symptomsToStartShedding, 2.1,
             sars.ci.dose$symptomsToStartShedding, col=sars.color,
             lwd=cex)
    ...
    # Confidence bounds
     polygon(c(0.63, 0.83, 0.83, 0.63), c(noro.ci.dose[noro.ci.dose$dose == 4.8,]
                                         $symptomsToStartSheddingLCI,
                                         noro.ci.dose[noro.ci.dose$dose == 4.8,]
                                         $symptomsToStartSheddingLCI,
                                         noro.ci.dose[noro.ci.dose$dose == 4.8,]
                                         $symptomsToStartSheddingUCI,
                                         noro.ci.dose[noro.ci.dose$dose == 4.8,]
                                         $symptomsToStartSheddingUCI),
            col=rgb(135, 26, 110, alpha=100,
                  maxColorValue=255), border = NA)
    ...
    polygon(c(1.9, 2.1, 2.1, 1.9), c(sars.ci.dose$symptomsToStartSheddingLCI,
                                         sars.ci.dose$symptomsToStartSheddingLCI,
                                         sars.ci.dose$symptomsToStartSheddingUCI,
                                         sars.ci.dose$symptomsToStartSheddingUCI),
            col=rgb(86, 179, 233, alpha=100,
                    maxColorValue=255), border = NA)
    ...
    
  {
    ...
    plot(symptomsToStartShedding ~ growthRate, data=params.combined, 
       col=ifelse(pathogen == 'Norovirus', noro.color, sars.color),
      pch=ifelse(pathogen == 'Norovirus', 
                    ifelse(symptomsToStartShedding < 0, 17, 2),
                    ifelse(symptomsToStartShedding < 0, 23, 5)),
    ...
    # Lines of best fit
    lines(symptomsToStartShedding ~ growthRate, data=noro.ci.growth, col=noro.color)
    lines(symptomsToStartShedding ~ growthRate, data=sars.ci.growth, col=sars.color)
    
    # Confidence bounds 
    polygon(c(noro.ci.growth$growthRate, rev(noro.ci.growth$growthRate)),
            c(noro.ci.growth$symptomsToStartSheddingUCI,
              rev(noro.ci.growth$symptomsToStartSheddingLCI)), 
            col = rgb(135, 26, 110, alpha=100,
                    maxColorValue=255), border = NA)
    
    polygon(c(sars.ci.growth$growthRate, rev(sars.ci.growth$growthRate)),
            c(sars.ci.growth$symptomsToStartSheddingUCI,
              rev(sars.ci.growth$symptomsToStartSheddingLCI)), 
            col=rgb(86, 179, 233, alpha=100,
                    maxColorValue=255), border = NA)
  ...
  {
    ylim.c <- c(floor(min(params.combined$sheddingOnset, na.rm=T)) - 1, 
                ceiling(max(params.combined$sheddingOnset, na.rm=T)) + 1)
    
    plot(sheddingOnset ~ growthRate, data=params.combined,
       col=ifelse(pathogen == 'Norovirus', noro.color, sars.color),
       xlim=xlim.bcd,
       ylim=ylim.c, cex.axis=cex, cex=cex.points,
       pch=ifelse(pathogen == 'Norovirus', 
    ...
    # Lines of best fit
    lines(sheddingOnset ~ growthRate, data=noro.ci.growth, col=noro.color)
    lines(sheddingOnset ~ growthRate, data=sars.ci.growth, col=sars.color)
    
    # Confidence bounds 
    polygon(c(noro.ci.growth$growthRate, rev(noro.ci.growth$growthRate)),
            c(noro.ci.growth$sheddingOnsetUCI, rev(noro.ci.growth$sheddingOnsetLCI)), 
          col=rgb(135, 26, 110, alpha=100,
                  maxColorValue=255), border = NA)
    
    polygon(c(sars.ci.growth$growthRate, rev(sars.ci.growth$growthRate)),
            c(sars.ci.growth$sheddingOnsetUCI, rev(sars.ci.growth$sheddingOnsetLCI)), 
            col=rgb(86, 179, 233, alpha=100,
                  maxColorValue=255), border = NA)
...
* examples of calculating viral load AUC prior to symptoms and prior to the
end of the first wave of infection
...
par(mfrow=c(3, 2), mar=c(3.4, 6, 1.1, 0.15))

{
  
  # Panel A: Norovirus example of calculating pre-symptomatic transmission potential
  plot.viralload(vldata=noro.FShedData[noro.FShedData$ID==exampleIDs[1],], 
                 pars=noro.example.params,
                 ...
                 panellabel="A",
                 example=TRUE,
                 auc.endpoint=noro.example.params$incubation,
                 detect.threshold=noro.detection.threshold)
  
  text(8, 
       6,
       labels="Viral load AUC\nbefore symptoms",
       col="#e69f00")
  
  
   # Panel B: Example of calculating transmission potential over the first wave of infection
  ...
  # Panel C: AUC before symptoms vs. day of symptom onset
  
  xrange <- c(floor(min(params.combined[
    !is.na(params.combined$transmissionBeforeSymptoms),]$incubation,
                  na.rm=T)) - 1,
              ceiling(max(params.combined[
    !is.na(params.combined$transmissionBeforeSymptoms),]$incubation,
                  na.rm=T)) + 1)
  yrange <- c(-5,
              ceiling(max(params.combined$transmissionBeforeSymptoms,
                  na.rm=T)) + 5)
  
  plot(transmissionBeforeSymptoms ~ incubation, 
       data=params.combined,
       col=ifelse(pathogen == 'Norovirus', noro.color, sars.color), 
       bg=ifelse(pathogen == 'Norovirus', noro.color, sars.color),
       pch=ifelse(pathogen == 'Norovirus', 
  ...
  axis(2, las = 2, cex.axis=1.2, mgp=c(0, 0.7, 0), tck=-0.04)
  
  # Lines of best fit
  lines(transmissionBeforeSymptoms ~ incubation, data=noro.ci.auc.incub, col=noro.color)
 
  lines(transmissionBeforeSymptoms ~ incubation, data=sars.ci.auc.incub, col=sars.color)
  
  # Confidence bounds
  polygon(c(noro.ci.auc.incub$incubation, rev(noro.ci.auc.incub$incubation)), c(noro.ci.auc.incub$transmissionBeforeSymptomsUCI,
                                                  rev(noro.ci.auc.incub$transmissionBeforeSymptomsLCI)), 
          col=rgb(135, 26, 110, alpha=100,
                    maxColorValue=255), border = NA)
  
  polygon(c(sars.ci.auc.incub$incubation, rev(sars.ci.auc.incub$incubation)), c(sars.ci.auc.incub$transmissionBeforeSymptomsUCI,
                                                  rev(sars.ci.auc.incub$transmissionBeforeSymptomsLCI)), 
          col=rgb(86, 179, 233, alpha=100,
                  maxColorValue=255), border = NA)
  
  mtext(text=expression(AUC~"of"~log[10](viral~load)), side=2, line=4, cex=0.9)
  mtext(text="before symptoms", side=2, line=3, cex=0.9)
  ...
  legend(-1.5, 55, 
         legend=c("Peak shedding before symptoms", "Peak shedding after symptoms"), 
     pch=c(19, 1), col=c("black", "black"), bty="n")
  
  text(xrange[1] + panellabelratio * (xrange[2] - xrange[1]), 
       panellabelratio * (yrange[2] - yrange[1]) + yrange[1], 
         labels="C", font=2)
  
  # Panel D: AUC during first wave of infection vs. end of first wave
  
  xrange <- c(floor(min(params.combined[!is.na(params.combined$transmissionFirstWave),]
                        $endFirstWave)) - 3,
              ceiling(max(params.combined[!is.na(params.combined$transmissionFirstWave),]
                          $endFirstWave)) + 1)
  yrange <- c(floor(min(params.combined$transmissionFirstWave, na.rm=T)) - 10,
              ceiling(max(params.combined$transmissionFirstWave, na.rm=T)) + 1)

  plot(transmissionFirstWave ~ endFirstWave, 
       data=params.combined,
       col=ifelse(pathogen == 'Norovirus', noro.color, sars.color), 
      pch=ifelse(pathogen == 'Norovirus', 
                  ifelse(symptomsToPeakDay < 0, 17, 2),
  ...
  mtext(text=expression(AUC~"of"~log[10](viral~load)~"for"), side=2, line=4, cex=0.9)
  mtext(text="first wave of infection", side=2, line=3, cex=0.9)
  mtext(text="Duration of first wave (days)", side=1, line=2, cex=0.9)
  
  # Lines of best fit
  lines(transmissionFirstWave ~ endFirstWave, data=noro.ci.auc.firstWave, col=noro.color)
  
  lines(transmissionFirstWave ~ endFirstWave, data=sars.ci.auc.firstWave, col=sars.color)
  
  arrows(x0 = noro.example.params$endFirstWave,
         y0 = noro.example.params$transmissionFirstWave + 22,
         x1 = noro.example.params$endFirstWave,
         y1 = noro.example.params$transmissionFirstWave + 10,
         length=0.05)
  
  # Confidence bounds
  polygon(c(noro.ci.auc.firstWave$endFirstWave, rev(noro.ci.auc.firstWave$endFirstWave)), 
          c(noro.ci.auc.firstWave$transmissionFirstWaveUCI, 
            rev(noro.ci.auc.firstWave$transmissionFirstWaveLCI)), 
          col=rgb(135, 26, 110, alpha=100,
                    maxColorValue=255), border = NA)
  
  polygon(c(sars.ci.auc.firstWave$endFirstWave, rev(sars.ci.auc.firstWave$endFirstWave)), 
          c(sars.ci.auc.firstWave$transmissionFirstWaveUCI, 
            rev(sars.ci.auc.firstWave$transmissionFirstWaveLCI)), 
          col=rgb(86, 179, 233, alpha=100,
                  maxColorValue=255), border = NA)
  ...
  # Panel E: AUC before symptoms vs. viral growth rate
  
  yrange=c(-10,
         ceiling(max(params.combined$transmissionBeforeSymptoms, na.rm=T)) + 1)
  
  plot(transmissionBeforeSymptoms ~ growthRate, data=params.combined,
    col=ifelse(pathogen == 'Norovirus', noro.color, sars.color),
    bg=ifelse(pathogen == 'Norovirus', noro.color, sars.color),
   pch=ifelse(pathogen == 'Norovirus', 
                  ifelse(symptomsToPeakDay < 0, 17, 2),
  ...
  lines(transmissionBeforeSymptoms ~ growthRate, data=noro.ci.growth, col=noro.color)

  lines(transmissionBeforeSymptoms ~ growthRate, data=sars.ci.growth, col=sars.color)
  ...
  polygon(c(noro.ci.growth$growthRate, rev(noro.ci.growth$growthRate)), 
          c(noro.ci.growth$transmissionBeforeSymptomsUCI, 
            rev(noro.ci.growth$transmissionBeforeSymptomsLCI)), 
          col=rgb(135, 26, 110, alpha=100,
                    maxColorValue=255), border = NA)
  
  polygon(c(sars.ci.growth$growthRate, rev(sars.ci.growth$growthRate)), 
          c(sars.ci.growth$transmissionBeforeSymptomsUCI, 
            rev(sars.ci.growth$transmissionBeforeSymptomsLCI)), 
          col=rgb(86, 179, 233, alpha=100,
                  maxColorValue=255), border = NA)
  ...
  plot(transmissionFirstWave ~ growthRate, data=params.combined,
    col=ifelse(pathogen == 'Norovirus', noro.color, sars.color),
    bg=ifelse(pathogen == 'Norovirus', noro.color, sars.color),
   pch=ifelse(pathogen == 'Norovirus', 
                  ifelse(symptomsToPeakDay < 0, 17, 2),
  ...
  lines(transmissionFirstWave ~ growthRate, data=noro.ci.growth, col=noro.color)
  
  lines(transmissionFirstWave ~ growthRate, data=sars.ci.growth, col=sars.color)
  
  arrows(x0 = noro.example.params$growthRate + 0.12,
         y0 = noro.example.params$transmissionFirstWave,
         x1 = noro.example.params$growthRate + 0.05,
         y1 = noro.example.params$transmissionFirstWave,
         length=0.05)
  
  # Confidence bounds
  
  polygon(c(noro.ci.growth$growthRate, rev(noro.ci.growth$growthRate)), 
          c(noro.ci.growth$transmissionFirstWaveUCI, 
            rev(noro.ci.growth$transmissionFirstWaveLCI)), 
          col=rgb(135, 26, 110, alpha=100,
                    maxColorValue=255), border = NA)
  
  polygon(c(sars.ci.growth$growthRate, rev(sars.ci.growth$growthRate)), 
          c(sars.ci.growth$transmissionFirstWaveUCI, 
            rev(sars.ci.growth$transmissionFirstWaveLCI)), 
          col=rgb(86, 179, 233, alpha=100,
                  maxColorValue=255), border = NA)
     ...
     width=3.5, height=6.5, unit='in')
{
  ...
  # Viral population growth rate vs. dose
  {
    ...
    segments(.65,
             noro.ci.dose[noro.ci.dose$dose == 4.8,]$growthRate, 
             .81, 
             noro.ci.dose[noro.ci.dose$dose == 4.8,]$growthRate,
             col=noro.color)
    segments(0.987,
             noro.ci.dose[noro.ci.dose$dose == 48,]$growthRate, 
             1.147, 
             noro.ci.dose[noro.ci.dose$dose == 48,]$growthRate,
             col=noro.color)
    segments(1.32,
             noro.ci.dose[noro.ci.dose$dose == 4800,]$growthRate, 
             1.48, 
             noro.ci.dose[noro.ci.dose$dose == 4800,]$growthRate,
             col=noro.color)
    ...
    # Confidence bounds
    polygon(c(.65, .81, .81, .65), c(noro.ci.dose[noro.ci.dose$dose==4.8,]$growthRateUCI,
                                     noro.ci.dose[noro.ci.dose$dose==4.8,]$growthRateUCI,
                                     noro.ci.dose[noro.ci.dose$dose==4.8,]$growthRateLCI,
                                     noro.ci.dose[noro.ci.dose$dose==4.8,]$growthRateLCI),
            col=rgb(135, 26, 110, alpha=100,
                  maxColorValue=255), border = NA)
    
    polygon(c(.987, 1.147, 1.147, .987), c(noro.ci.dose[noro.ci.dose$dose==48,]$growthRateUCI,
                                     noro.ci.dose[noro.ci.dose$dose==48,]$growthRateUCI,
                                     noro.ci.dose[noro.ci.dose$dose==48,]$growthRateLCI,
                                     noro.ci.dose[noro.ci.dose$dose==48,]$growthRateLCI),
            col=rgb(135, 26, 110, alpha=100,
                  maxColorValue=255), border = NA)
    
    polygon(c(1.32, 1.48, 1.48, 1.32), c(noro.ci.dose[noro.ci.dose$dose==4800,]$growthRateUCI,
                                     noro.ci.dose[noro.ci.dose$dose==4800,]$growthRateUCI,
                                     noro.ci.dose[noro.ci.dose$dose==4800,]$growthRateLCI,
                                     noro.ci.dose[noro.ci.dose$dose==4800,]$growthRateLCI),
            col=rgb(135, 26, 110, alpha=100,
                  maxColorValue=255), border = NA)
  ...
  
  {
    ...
    axis(1, at=c(.73, 1.067, 1.4), labels = c(4.8, 48, 4800), mgp=c(0, 0.75, 0),
         tck=-0.04, cex.axis=cex)
     segments(.65,
             noro.ci.dose[noro.ci.dose$dose == 4.8,]$peakDay,
             .81, 
             noro.ci.dose[noro.ci.dose$dose == 4.8,]$peakDay,
             col=noro.color)
    segments(.987,
             noro.ci.dose[noro.ci.dose$dose == 48,]$peakDay,
             1.147, 
             noro.ci.dose[noro.ci.dose$dose == 48,]$peakDay,
             col=noro.color)
    segments(1.32,
             noro.ci.dose[noro.ci.dose$dose == 4800,]$peakDay, 
             1.48, 
             noro.ci.dose[noro.ci.dose$dose == 4800,]$peakDay,
             col=noro.color)
    ...
    # Confidence bounds
    polygon(c(.65, .81, .81, .65), c(noro.ci.dose[noro.ci.dose$dose==4.8,]$peakDayUCI,
                                     noro.ci.dose[noro.ci.dose$dose==4.8,]$peakDayUCI,
                                     noro.ci.dose[noro.ci.dose$dose==4.8,]$peakDayLCI,
                                     noro.ci.dose[noro.ci.dose$dose==4.8,]$peakDayLCI),
            col=rgb(135, 26, 110, alpha=100,
                  maxColorValue=255), border = NA)
    
    polygon(c(.987, 1.147, 1.147, .987), c(noro.ci.dose[noro.ci.dose$dose==48,]$peakDayUCI,
                                     noro.ci.dose[noro.ci.dose$dose==48,]$peakDayUCI,
                                     noro.ci.dose[noro.ci.dose$dose==48,]$peakDayLCI,
                                     noro.ci.dose[noro.ci.dose$dose==48,]$peakDayLCI),
            col=rgb(135, 26, 110, alpha=100,
                  maxColorValue=255), border = NA)
    
    polygon(c(1.32, 1.48, 1.48, 1.32), c(noro.ci.dose[noro.ci.dose$dose==4800,]$peakDayUCI,
                                     noro.ci.dose[noro.ci.dose$dose==4800,]$peakDayUCI,
                                     noro.ci.dose[noro.ci.dose$dose==4800,]$peakDayLCI,
                                     noro.ci.dose[noro.ci.dose$dose==4800,]$peakDayLCI),
            col=rgb(135, 26, 110, alpha=100,
                  maxColorValue=255), border = NA)
  }
  ...
  
  {
    ...
    axis(1, at=c(.73, 1.067, 1.4), labels = c(4.8, 48, 4800), mgp=c(0, 0.75, 0),
       ...
       segments(.65,
             noro.ci.dose[noro.ci.dose$dose == 4.8,]$incubation, 
             .81, 
             noro.ci.dose[noro.ci.dose$dose == 4.8,]$incubation,
             col=noro.color)
       segments(.987,
             noro.ci.dose[noro.ci.dose$dose == 48,]$incubation, 
             1.147, 
             noro.ci.dose[noro.ci.dose$dose == 48,]$incubation,
             col=noro.color)
    segments(1.32,
             noro.ci.dose[noro.ci.dose$dose == 4800,]$incubation, 
             1.48, 
             noro.ci.dose[noro.ci.dose$dose == 4800,]$incubation,
             col=noro.color)
    ...
    # Confidence bounds
    polygon(c(.65, .81, .81, .65), c(noro.ci.dose[noro.ci.dose$dose==4.8,]$incubationUCI,
                                     noro.ci.dose[noro.ci.dose$dose==4.8,]$incubationUCI,
                                     noro.ci.dose[noro.ci.dose$dose==4.8,]$incubationLCI,
                                     noro.ci.dose[noro.ci.dose$dose==4.8,]$incubationLCI),
            col=rgb(135, 26, 110, alpha=100,
                  maxColorValue=255), border = NA)
    
    polygon(c(.987, 1.147, 1.147, .987), c(noro.ci.dose[noro.ci.dose$dose==48,]$incubationUCI,
                                     noro.ci.dose[noro.ci.dose$dose==48,]$incubationUCI,
                                     noro.ci.dose[noro.ci.dose$dose==48,]$incubationLCI,
                                     noro.ci.dose[noro.ci.dose$dose==48,]$incubationLCI),
            col=rgb(135, 26, 110, alpha=100,
                  maxColorValue=255), border = NA)
    
    polygon(c(1.32, 1.48, 1.48, 1.32), c(noro.ci.dose[noro.ci.dose$dose==4800,]$incubationUCI,
                                     noro.ci.dose[noro.ci.dose$dose==4800,]$incubationUCI,
                                     noro.ci.dose[noro.ci.dose$dose==4800,]$incubationLCI,
                                     noro.ci.dose[noro.ci.dose$dose==4800,]$incubationLCI),
            col=rgb(135, 26, 110, alpha=100,
                  maxColorValue=255), border = NA)
